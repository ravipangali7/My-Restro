...
export const Route = createFileRoute("/owner/notifications_/$id")({ component: OwnerNotificationViewPage });

function OwnerNotificationViewPage() {
  ...
  const showFollowLink = useMemo(() => {
    if (!notif || role !== "owner") return false;
    const home = portalHomeByRole.owner;
    const resolved = safeBulkNotificationTargetLink(notif.link, "owner");
    if (resolved === home) return false;
    const nav = bulkNotificationNavigateTarget(notif.link, "owner");
    const selfPath = `/owner/notifications/${id}`;
    if (nav.to === selfPath) return false;
    return true;
  }, [notif, role, id]);

  const followLinkedPage = () => {
    if (!notif || role !== "owner") return;
    const nav = bulkNotificationNavigateTarget(notif.link, "owner");
